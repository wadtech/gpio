...
#define THERM_SERIAL "28-00141186abff"
...
int main(void)
{
	...
	for(;;) {
		...
		reading_t *raw = thermRead(THERM_SERIAL);
		if (raw == NULL) {
			//something is fucky
		...
		printf("current reading is: %2.3fC, %2.3fF", degrees_c, degrees_f);
...
int setUp()
{
	...
	if (thermEnabled(THERM_SERIAL) == FALSE) {
		return FALSE;
	}
